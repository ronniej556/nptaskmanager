...
if(isset($_POST['take_task']))
{
  ...
  $q->execute(array('in-progress', $task['id']));
  ...
  $q = $pdo->prepare('INSERT INTO `developer_tasks` VALUES (?,?,?,?,?)');
  $q->execute(array($_SESSION['developer']['id'], $task['id'], 0, 0, NULL));
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container" style="margin-top: 20px; max-width: 1400px;">
      ...
      $developer_name = explode(' ', $_SESSION['developer']['name'])[0].' '.substr(explode(' ', $_SESSION['developer']['name'])[1], 0, 1).'.';
      ...
      if(isset($_POST['reply']))
      {
        ...
        $q->execute(array($task['id'], $developer_name, $_POST['reply'], time(), NULL));

        header('Location: developer_task.php?id='.$task['id']);
        exit();
      ...
      <div class="float-right">
        ...
        if($task['status'] == 'active')
        {
          ...
          <form action="" method="post">
            <button type="submit" name="take_task" class="btn btn-primary btn-lg">
              TAKE TASK
            </button>
          </form>
          <?php
