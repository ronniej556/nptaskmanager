...
<html lang="en">
  <head>
    ...
    <title>NerdPilots - Administration</title>
    <style type="text/css">
  ...
  <body>
    ...
    <div class="container" style="margin-top: 20px; max-width: 1400px;">
      ...
      <div class="row" style="margin-top: 20px;">
            ...
            <div class="card-body">
              ...
              foreach ($q as $row) {
                ...
                if($updated->rowcount()>0)
                {
                  ...
                  echo '
                  <a class="btn btn-primary btn-block text-left" href="admin_task.php?id='.$row['id'].'">
                ...
                {
                  echo '
                  <a class="btn btn-secondary btn-block text-left" href="admin_task.php?id='.$row['id'].'">
                      '.$row['title'].'
                      <span class="badge badge-default float-right">by '.explode(' ', $memberInfo['name'])[0].' '.substr(explode(' ', $memberInfo['name'])[1], 0, 1).'.'.'</span>
                    </a>
                  ';
        ...
        <div class="col-md-6">
          <?php
          $q = $pdo->prepare('SELECT * FROM `developer_tasks` WHERE `completed`=? ORDER BY `id` DESC');
          $q->execute(array('0'));
          ?>
          <h4>Tasks In-Progress <span class="badge badge-success"><?php echo $q->rowcount(); ?></span></h4>
          <div class="card" style="height: 50vh; overflow-y: scroll;">
            <div class="card-body">
              ...
              foreach ($q as $row) {
                ...
                if($updated->rowcount()>0)
                {
                  $updated = $updated->fetch(PDO::FETCH_ASSOC);
                  echo '
                  <a class="btn btn-primary btn-block text-left" href="admin_task.php?id='.$task['id'].'">
                ...
                {
                  echo '
                  <a class="btn btn-secondary btn-block text-left" href="admin_task.php?id='.$task['id'].'">
                      '.$task['title'].'
                      <span class="badge badge-default float-right">by '.explode(' ', $memberInfo['name'])[0].' '.substr(explode(' ', $memberInfo['name'])[1], 0, 1).'.'.'</span>
                    </a>
                  ';
            ...
            <div class="card-body">
              ...
              foreach ($q as $row) {
                ...
                if($updated->rowcount()>0)
                {
                  ...
                  echo '
                  <a class="btn btn-primary btn-block text-left" href="admin_task.php?id='.$task['id'].'">
                      '.$task['title'].' <span class="badge badge-default float-right">Updated '.time_elapsed_string('@'.$updated['time']).' / Completed by '.explode(' ', $developer['name'])[0].' '.substr(explode(' ', $developer['name'])[1], 0, 1).'.'.'</span>
                    </a>
                  ';
                }
                else
                {
                  echo '
                  <a class="btn btn-secondary btn-block text-left" href="admin_task.php?id='.$task['id'].'">
                      '.$task['title'].' <span class="badge badge-default float-right">Completed by '.explode(' ', $developer['name'])[0].' '.substr(explode(' ', $developer['name'])[1], 0, 1).'.'.'</span>
                    </a>
                  ';
                }
